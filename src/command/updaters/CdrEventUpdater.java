  ...
  extends AbstractEventUpdater
{
  ...
  public void update() throws UpdateDataException
  {
    ...
    createParams.add("CdrEvent");
    createParams.add(event.getSrc());
    createParams.add(event.getDestination());
    createParams.add(event.getDisposition());
    
    System.out.println("command.updaters.CdrEventUpdater.update() nombre del evento: " + event.getClass().getSimpleName());
    ...
    {
      if ((recordingPath != null) && (!recordingPath.equals(""))) {
        database.updateSql("UPDATE " + SQLiteDatabase.callsTable + " SET endtime = ?, starttime = ?, totalduration = ?, event = ?, from_number = ?, to_number = ?, disposition = ?, recordingpath = ?, recordingurl = ? WHERE uid=?;", createParams);
      }
      else
      {
        database.updateSql("UPDATE " + SQLiteDatabase.callsTable + " SET endtime = ?, starttime = ?, totalduration = ?, event = ?, from_number = ?, to_number = ?, disposition = ? WHERE uid=?;", createParams);
      }
    }
