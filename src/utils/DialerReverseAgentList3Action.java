// Source File Name: DialerReverseAgentList3Action.java
package utils;
...
public class DialerReverseAgentList3Action {

    private static final Logger logger = Logger.getLogger(DialerReverseAgentList3Action.class);
    protected SQLiteDatabase database;
    private final String context;
    private final String queue;
    private final ManagerConnection connection;

    public DialerReverseAgentList3Action(ManagerConnection connection) {
    ...
    public String doAction() throws SQLException, ParseException {
        ...
        while (rs.next()) {
            ...
            Integer camp_tiempofuera = rs.getInt("camp_tiempofuera");
            ...
            if (targetInDay && (targetInMorning || targetInAfternoon)) {
                ...
                ResultSet rsa = database.selectSql("SELECT a.uid as uid, a.agen_id as agen_id, a.agen_exten as agen_exten, a.agen_lastendcall FROM " + SQLiteDatabase.agentTable + " a"
                ...
                while (rsa.next()) {
                    ...
                    Calendar agen_lastendcall = Calendar.getInstance();
                    Calendar agen_nextcall = Calendar.getInstance();
                    Calendar horaactual = Calendar.getInstance();
                    Long lastendcall = rsa.getLong("agen_lastendcall");
                    agen_nextcall.setTimeInMillis(lastendcall);
                    agen_nextcall.add(Calendar.SECOND, camp_tiempofuera);
                    if (horaactual.getTimeInMillis() < agen_nextcall.getTimeInMillis()) {
                        System.out.println("Agente en tiempo fuera...");
                        continue;
                    }
                    params.clear();
                    ...
                    System.out.println("Verificando si hay llamadas pendientes para el agente...");
                    while (rsl.next()) {
                        hayEnL1 = true;
                        ...
                        database.updateSql("UPDATE " + SQLiteDatabase.agentTable + " SET agen_estado = ?, agen_laststartcall = ? WHERE uid = ?", aparams);
                        ...
                        try {
                            ManagerResponse ex = this.connection.sendAction(action, 5000L);
